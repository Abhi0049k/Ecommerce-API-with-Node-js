...
orderRouter.post('/placeOrder/:productId', userAuth(['admin', 'customer']), async (req, res) => {
    try {
        const { user } = req.body;
...
orderRouter.delete('/removeProduct/:productId', userAuth(['admin', 'customer']), async (req, res) => {
    try {
        const { user } = req.body;
        const { productId } = req.params;
        const order = await orderModel.findOne({ userId: user._id, status: 'pending' });
...
orderRouter.patch('/increaseQty/:productId', userAuth(['admin', 'customer']), async (req, res) => {
    try {
        const { user } = req.body;
        const { productId } = req.params;
        const order = await orderModel.findOne({ userId: user._id, status: 'pending' });
...
orderRouter.patch('/decreaseQty/:productId', userAuth(['admin', 'customer']), async (req, res) => {
    try {
        const { user } = req.body;
        const { productId } = req.params;
        const order = await orderModel.findOne({ userId: user._id, status: 'pending' });
...
orderRouter.delete('/removeOrder/:orderId', userAuth(['admin', 'customer']), async(req, res)=>{
    try{
        const {user} = req.body;
